...
function isContainNumber(num, digit) {
  ...
  while (counter <= num * 10) {
    if ((mutatedNum % counter) / (counter / 10) === digit) return true;
    mutatedNum -= mutatedNum % counter;
    counter *= 10;
  }
  return false;
}
...
function getBalanceIndex(arr) {
  let leftSumm = arr[0];
  let leftCounter = 0;
  while (leftCounter < arr.length - 1) {
    leftCounter += 1;
    leftSumm += arr[leftCounter];
    let rightSumm = 0;
    for (let i = arr.length - 1; i > leftCounter + 1; i -= 1) {
      rightSumm += arr[i];
    }
    if (rightSumm === leftSumm) return leftCounter + 1;
  }
  return -1;
}
...
function getSpiralMatrix(size) {
  ...
  let endRow = size - 1;
  let startColummn = 0;
  let endColummn = size - 1;

  while (startRow <= endRow && startColummn <= endColummn) {
    for (let i = startRow; i <= endRow; i += 1) {
      resultArr[startRow][i] = count;
      count += 1;
    }
    startRow += 1;

    for (let i = startRow; i <= endRow; i += 1) {
      resultArr[i][endColummn] = count;
      count += 1;
    }
    endColummn -= 1;

    for (let i = endColummn; i >= startColummn; i -= 1) {
      resultArr[endRow][i] = count;
      count += 1;
    }
    endRow -= 1;

    for (let i = endRow; i >= startRow; i -= 1) {
      resultArr[i][startColummn] = count;
      count += 1;
    }
    startColummn += 1;
  }
...
function shuffleChar(str, iterations) {
  ...
  while (count < iterations) {
    ...
    for (let i = 0; i < str.length; i += 1) {
      if (i % 2 === 0) {
        firstPart += resultString[i];
      } else {
        lastPart += resultString[i];
      }
    }
